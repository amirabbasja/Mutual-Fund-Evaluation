...
B. Portfolio Based:
    1.	Value at Risk
        Value at Risk (VaR) is a statistical measure used to quantify the potential loss in value of a portfolio or an investment over a specific time period and with a certain level of confidence. It provides a single number that indicates the extent of risk in a given portfolio, measured in either price units or as a percentage. The formula for VaR can be computed using various methods, including the historical method, variance-covariance method, and Monte Carlo method. One common formula for VaR is the parametric method, which is calculated as a function of the mean and variance of the returns series, assuming a normal distribution. Another approach is the historical method, where VaR is determined by taking the returns belonging to the lowest quintile of the series. The Monte Carlo method involves simulating a large number of scenarios for the portfolio and determining VaR based on the worst losses.    
        In summary, VaR is a widely used risk assessment technique that provides a measure of the potential loss that an asset, portfolio, or firm might experience over a given period of time. It is an important tool for risk management and is used by financial institutions to assess and control the level of risk exposure in their portfolios. The interpretation of VaR is that it signifies the maximum possibility of a loss in a given period or as a measure of the magnitude or potential of losses in a portfolio

    2.	Risk Stability 
        Risk Stability (Rs) is the Investable Attribute that measures the stability of the risk assumed across the positions that form a trading strategy. The higher the Rs score, the more stable the trading strategy's risk. A good Rs score does not necessarily imply conservative money management. Risk Stability is calculated based on VaR.

    3.	Loss Aversion
        Loss Aversion (La) is an Investable Attribute that evaluates whether a trading strategy exhibits symmetric behavior independently of whether a position is winning or losing. It measures the tendency of a trader to let losses run and close winning positions quickly, which is a cognitive bias considered extremely dangerous, especially when combined with significant levels of leverage. The La score is scored from 0-10, with a higher score indicating a more favorable attribute.
        
    4.  Sector Allocation, Stock Selection, Interaction (A):
        i. (Brinson–Hood–Beebower (1986) Model)
        a. Allocation: In return attribution, allocation refers to the value the portfolio manager adds by
        having different sector weights in the portfolio than the sector weights in the benchmark. A sector weight in the portfolio greater than the equivalent benchmark sector
        ...
        common, label.
        ![alt text](https://latex.codecogs.com/svg.image?%5Csum_%7Bi=1%7D%5E%7Bi=n%7D(w_%7Bi%7D-W_%7Bi%7D)B_%7Bi%7D)
        where:
        . w is portfolio Weight
        . W is benchmark weight
        . B is benchmark return
        b. Selection: Selection refers to the value the portfolio manager adds by holding individual
        ...
        ![alt text](https://latex.codecogs.com/svg.image?%5Csum_%7Bi=1%7D%5E%7Bi=n%7D(R_%7Bi%7D-B_%7Bi%7D)W_%7Bi%7D)
        where:
        . W is benchmark weight
        . B is benchmark return
        . R is portfolio return
        c. Interaction: The interaction effect in the context of performance attribution analysis is the combination of the selection and allocation effects. It represents the cumulative impact created by asset allocation, security selection, and other investment decisions made by the portfolio manager. If the portfolio allocation outweighs and outperforms the benchmark, the interaction effect is positive, and vice versa. However, it is not easily attributable due to the complexity of the factors involved. In the Brinson-Hood-Beebower (BHB) model, the interaction effect is included, although it is only sometimes reported. It is an important component in evaluating a portfolio manager's skill in selecting securities and allocating assets
        ![alt text](https://latex.codecogs.com/svg.image?%5Csum_%7Bi=1%7D%5E%7Bi=n%7D(w_%7Bi%7D-W_%7Bi%7D)(R_%7Bi%7D-B_%7Bi%7D))
        where:
        . W is benchmark weight
        . w is portfolio Weight
        . B is benchmark return
        ...
        effects. Clearly, if the portfolio manager is overweight in a negative market that has outperformed the overall benchmark, the effect should be positive.
        ...
        a. Allocation:
        ![alt text](https://latex.codecogs.com/svg.image?%5Csum_%7Bi=1%7D%5E%7Bi=n%7D(w_%7Bi%7D-W_%7Bi%7D)(B_%7Bi%7D-B))
        b. Selection: like BHB model
        c. Interaction: like BHB model
        iii. Allen (1991), Bain (1996), Burnie, Knowles, and Teder (1998) (Geometic formula)
        a. Allocation:
        ![alt text](https://latex.codecogs.com/svg.image?%5Csum_%7Bi=1%7D%5E%7Bi=n%7D(w_%7Bi%7D-W_%7Bi%7D)(%5Cfrac%7B1&plus;B_i%7D%7B1&plus;B%7D-1))
        b. Selection:
        ![alt text](https://latex.codecogs.com/svg.image?%5Csum_%7Bi=1%7D%5E%7Bi=n%7D(w_%7Bi%7D)(%5Cfrac%7B1&plus;R_i%7D%7B1&plus;B_i%7D-1)%5Cfrac%7B(1&plus;B_i)%7D%7B(1&plus;B_S)%7D)
        where:
        ![alt text](https://latex.codecogs.com/svg.image?B_S=%5Csum_%7Bi=1%7D%5E%7Bi=n%7D(w_iB_i))
        c. Interaction:
    5. GT measure:
        The Grinblatt and Titman (GT) measure is a mutual fund performance measure that uses the past portfolio weights of a fund as a benchmark. It is one of the characteristic-based performance measures developed by Grinblatt and Titman in 1993. 
        ![alt text](https://latex.codecogs.com/svg.image?GT=%5Csum_%7Bj=1%7D%5E%7BN%7Dcov(w%7B_%7Bj,t-1%7D%7D(R%7B_%7Bj,t%7D%7D)))
        The GT measure can also be decomposed into style bets, style timing bets, market timing bets, industry bets, etc.
        i. stockpicking bets
        ![alt text]( https://latex.codecogs.com/svg.image?GT_t=%5Csum_%7Bj=1%7D%5E%7BN%7D(w%7B_%7Bj,t-1%7D%7D-w%7B_%7Bj,t-1%7D%7D)R_%7Bj,t%7D)
       
        ii. industry bets and stock picking from industry
        ![alt text](https://latex.codecogs.com/svg.image?GT_t=%5Csum_%7Bj=1%7D%5E%7BN%7D(w%7B_%7Bj,t-1%7D%7D-w%7B_%7Bj,t-1%7D%7D)R_%7Bt%7D%5E%7BIND(j)%7D&plus;%5Csum_%7Bj=1%7D%5E%7BN%7D(w%7B_%7Bj,t-1%7D%7D-w%7B_%7Bj,t-1%7D%7D)(R_%7Bj,t%7D-R_%7Bt%7D%5E%7BIND(j)%7D))
    6. DGTW measure
        The Daniel, Grinblatt, Titman and Wermers (DGTW, 1997) measure is a mutual fund performance measure that uses the past portfolio weights of a fund as a benchmark.
        The DGTW measure can also be decomposed into:
        a.	Characteristic Selectivity (CS):
            This measure of stock-picking ability, which is called th “Characteristic-Selectivity” measure (CS), is computed during month t as:

            ![alt text](https://latex.codecogs.com/svg.image?CS_t=%5Csum_%7Bj=1%7D%5E%7BN%7Dw%7B_%7Bj,t-1%7D%7D(R%7B_%7Bj,t%7D%7D-R_%7Bt%7D%5E%7Bb_j,t-1%7D))

            where $w{_{j,t-1}}$ is the portfolio weight on stock j at the end of month t − 1, $R{_{j,t}}$ is the month t buy-and-hold return of stock j, and $R_{t}^{b_j,t-1}$ is the month t buy-andhold return of a value-weighted portfolio that is matched to stock j based on its characteristics at the beginning of the quarter.
            The time-series average, over all months that a fund exists, gives the CS measure for that fund.
            To construct the characteristic-matched benchmark portfolio for a given stock at the beginning of a given quarter, we characterize that stock over three
            dimensions—the market capitalization of equity (size), the ratio of book-value of equity to market-value of equity, and the prior-year return. Forming these
            matching portfolios proceeds as follows—this procedure is based on DGTW, and is described in more detail in that paper. First, all stocks (listed on NYSE,
            AMEX, or Nasdaq) having book value of equity information in Compustat, and stock return and market capitalization of equity data in the Center for Research
            in Security Prices (CRSP) stock files, are ranked, at the end of each June, by their market capitalization. Quintile portfolios are formed (using NYSE size quintile breakpoints), and each quintile portfolio is further subdivided into bookto-market quintiles, based on their book-to-market data as of the end of the
            December immediately prior to the ranking year. Finally, each of the resulting 25 fractile portfolios is further subdivided into quintiles based on the 12-month
            past return of stocks through the end of May of the ranking year. This three-wayranking procedure results in 125 fractile portfolios, each having a distinct combination of size, book-to-market, and momentum characteristics.18 The threeway ranking procedure is repeated at the end of June of each year, and the 125 portfolios are reconstituted at that date. Non-June 30 quarter-end rankings are
            done similarly, except that book-to-market data are not updated. Value-weighted returns are computed for each of the 125 fractile portfolios, and
            the benchmark for each stock during a given month is the buy-and-hold return of the fractile portfolio of which that stock is a member during that month (available by Googling “DGTW benchmarks”). Therefore, the characteristic-adjusted return for a given stock is computed as the buy-and-hold stock return minus the buy-and-hold (value-weighted) matched benchmark return during the same
            month. Finally, the Characteristic Selectivity measure of the stock portfolio of a given mutual fund during month t, CSt, is computed as the portfolio-weighted
            characteristic-adjusted return of the component stocks in the portfolio, where the stock portfolio is normalized so that the weights add to one (to focus on the
            performance of the equity portion of the portfolio). A caveat is in order regarding the interpretation of the CS measure, as it controls for only three characteristic dimensions of stocks—size, book-to-market, and past returns. Recent research has shown that mutual funds show a distinct preference for other stock characteristics that are related to average returns—
            for example, stocks with greater liquidity (see Chen et al., 2000 and Wermers, 2000).19 For example, one might argue that our CS measure underestimates
            the stock-picking talents of funds since we do not control for the lower average returns that accrue to stocks with greater liquidity. These potential “missing factors” probably do not impact the practical uses of the DGTW approach, but the literature has yet to fully explore such issues.
        b.	Characteristic Timing (CT):
            The above stock-selectivity measure does not capture the ability of the fund manager to time the various stock characteristics. Indeed, fund managers can
            generate additional performance if size, book-to-market, or momentum strategies have time-varying expected returns that the manager can exploit by “tilting” portfolio weights toward stocks having these characteristics when the returns on the characteristics are highest. Thus, our second component of performance measures a fund manager’s success at timing the different stock characteristics; this component is termed the “Characteristic Timing” (CT) measure. The month t component of this measure is:

            ![alt text](https://latex.codecogs.com/svg.image?CT_t=%5Csum_%7Bj=1%7D%5E%7BN%7D(w%7B_%7Bj,t-1%7D%7DR_%7Bt%7D%5E%7Bb%7B_%7Bj,t-1%7D%7D%7D-w%7B_%7Bj,t-k-1%7D%7DR_%7Bt%7D%5E%7Bb%7B_%7Bj,t-k-1%7D%7D%7D))

        c.  Average Style (AS)
            To measure the returns earned by a fund because of that fund’s tendency to hold
            stocks with certain characteristics, we employ our third performance component,
            the “Average Style” (AS) return measure. The month t component of this measure is:
            
            ![alt text](https://latex.codecogs.com/svg.image?AS_t=%5Csum_%7Bj=1%7D%5E%7BN%7Dw%7B_%7Bj,t-k-1%7D%7DR_%7Bt%7D%5E%7Bb%7B_%7Bj,t-k-1%7D%7D%7D)
            The AS measure of a fund may differ from the return on a broad market index
            for a couple of reasons. First, the AS measure may contain a compensation for
            the fund loading on covariance-based risk factors differently than the market
            portfolio’s loadings. And, second, the AS measure may contain return premia
            for the fund loading on non-covariance-based characteristic factors.
        d.  gross return
            the sum of the CS, CT, and AS measures equals the total portfolioweighted (pre-expense and pre-trade cost) return on the stockholdings of a given fund (we also call this the “gross return” of the fund). That is:
            ![alt text](https://latex.codecogs.com/svg.image?GR_t=CS_t&plus;CT_t&plus;AS_t)
